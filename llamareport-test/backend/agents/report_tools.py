        ...
        if hasattr(response, 'raw'):
            ...
            # 如果是字符串，尝试解析 JSON
                ...
                try:
                    ...
                    return {"error": "响应格式错误", "raw": raw_data}
            else:
                # 其他类型，尝试转换为字典
                logger.warning(f"意外的响应类型: {type(raw_data)}")
                return {"error": "响应类型错误", "type": str(type(raw_data))}
        else:
            # 如果没有 raw 属性，尝试直接使用 response
            logger.warning("响应没有 raw 属性，尝试直接使用 response")
            if hasattr(response, 'model_dump'):
                return response.model_dump()
            elif isinstance(response, dict):
                return response
            else:
                return {"error": "无法处理响应", "response_type": str(type(response))}
        ...
        if hasattr(response, 'raw'):
            ...
            if hasattr(raw_data, 'model_dump'):
                ...
                try:
                    ...
                    logger.warning(f"无法解析字符串响应为JSON")
                    return {"error": "响应格式错误"}
            else:
                logger.warning(f"意外的响应类型: {type(raw_data)}")
                return {"error": "响应类型错误"}
