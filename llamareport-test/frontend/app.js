...
(function() {
    ...
    props: { chartData: { type: Object, default: null }, loading: { type: Boolean, default: false } },
    computed: {
        status() {
            if (this.loading) return 'loading';
            ...
            this.$nextTick(() => {
                try {
                    ...
                    const traces = chartConfig.traces.map(trace => {
                        const plotlyTrace = { type: trace.type || 'scatter', name: trace.name || '数据' };
                        if (trace.type === 'pie') {
                    ...
                    const layout = {
                        title: { text: chartConfig.layout.title || '', font: { size: 16, color: '#333' } },
                        xaxis: { title: chartConfig.layout.xaxis_title || '', gridcolor: '#e0e0e0' },
                        yaxis: { title: chartConfig.layout.yaxis_title || '', gridcolor: '#e0e0e0' },
                        height: 400,
    ...
    watch: {
        chartData: { handler() { if (this.chartData) this.renderChart(); }, deep: true }
    },
    template: `
